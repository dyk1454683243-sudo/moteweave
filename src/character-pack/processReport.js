...
export function buildDebugReport({
  ...
  return {
    ...
    requested_background_mode: options.backgroundRequestMode ?? options.background_request_mode ?? options.backgroundMode ?? background.requested_mode ?? 'auto',
    canonical_background_mode: background.canonical_mode ?? null,
    background_recipe_id: background.recipe_id ?? null,
    ...
    background_processed_before_geometry: background.processed_before_geometry === true,
    alpha_provenance: background.alpha_provenance ?? 'unknown',
    input_alpha: background.input_alpha ?? null,
    source_decode: background.input_decode ?? null,
